...
String lecture_name = ((String)request.getAttribute("name"));
...
<div class="wrap">
	...
	<td colspan="5">
	    <div class="pagination">
            ...
            <%}else{ %> 
                <a href ="./LectureList.le?page=<%=nowpage-1%>&lecture_name=<%= lecture_name%>">Prev</a>&nbsp; 
            <%} %> 
             
            <%for(int a=startpage ; a<=endpage ; a++){  
                    if(a==nowpage){ %>  
                       <a href="./LectureList.le?page=<%=a %>&lecture_name=<%= lecture_name%>"><strong><%=a%></strong></a>&nbsp;  
               <%}else{ %> 
                <a href="./LectureList.le?page=<%=a %>&lecture_name=<%= lecture_name%>"><strong><%=a%></strong></a>&nbsp; 
                <%} %> 
             <%} %>  
              
             <%if(nowpage>=maxpage){ %> <a href ="./LectureList.le?page=<%=nowpage+1%>&lecture_name=<%= lecture_name%>" class="direction"><span>Next</span>
             </a>
            <%}else{ %> 
                <a href ="./LectureList.le?page=<%=nowpage+1%>&lecture_name=<%= lecture_name%>">Next</a> 
            <%} %>
