...
<div class="wrap">
    ...
    <tr height="1" bgcolor="#e5e5e5"><td colspan="5"></td></tr>
        <td colspan="4" align="center">Q&A게시판</td> 
        ...
        <td><%=bl.getBOARD_NUM() %></td> 
		 ...
		 if(bl.getBOARD_RE_LEV()>0){ 
       	%>
            ...
            <a href="./QADetailAction.bo?num=<%=bl.getBOARD_NUM() %>"> 
	...
	<div class="pagination">
            ...
            <%if(nowpage<=1){ %> <a href ="./QAList2.bo?page=<%=nowpage-1%>" class="direction"><span>Prev</span>
            </a>&nbsp;         
            <%}else{ %> 
                <a href ="./QAList2.bo?page=<%=nowpage-1%>&board_subject=<%= board_subject%>">Prev</a>&nbsp; 
            <%} %> 
             
            <%for(int a=startpage ; a<=endpage ; a++){  
                    if(a==nowpage){ %>  
                       <a href="./QAList2.bo?page=<%=a %>&board_subject=<%= board_subject%>"><strong><%=a%></strong></a>&nbsp;  
               <%}else{ %> 
                <a href="./QAList2.bo?page=<%=a %>&board_subject=<%= board_subject%>"><strong><%=a%></strong></a>&nbsp; 
                <%} %> 
             <%} %>  
              
             <%if(nowpage>=maxpage){ %> <a href ="./QAList2.bo?page=<%=nowpage+1%>&board_subject=<%= board_subject%>" class="direction"><span>Next</span>
             </a>
            <%}else{ %> 
                <a href ="./QAList2.bo?page=<%=nowpage+1%>&board_subject=<%= board_subject%>">Next</a> 
            <%} %>
   ...
   <tr align="right"> 
        <td colspan="5"> 
			<% String boardsubject = request.getParameter("board_subject1"); %>    
        		<form action="./QAList2.bo?page=<%=nowpage%> method="POST">
