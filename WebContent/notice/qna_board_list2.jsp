...
<div class="wrap">
    ...
    <tr height="1" bgcolor="#e5e5e5"><td colspan="5"></td></tr>
        <td colspan="4" align="center">공지사항</td> 
    ...
    <tr align="center" valign="middle"   > 
        ...
        <td> 
            ...
            <%} %> 
            <a href="./NoticeDetailAction.bo?num=<%=bl.getBOARD_NUM() %>"> 
                <strong><%=bl.getBOARD_SUBJECT() %></strong></a></div>                 
        </td>         
        <td> 
            <div align="center"><strong>관리자</strong></div> 
        </td> 
        <td> 
	...
	<div class="pagination">
            ...
            <%if(nowpage<=1){ %> <a href ="./NoticeList2.bo?page=<%=nowpage-1%>" class="direction"><span>Prev</span>
            </a>&nbsp;         
            <%}else{ %> 
                <a href ="./NoticeList2.bo?page=<%=nowpage-1%>&board_subject=<%= board_subject%>">Prev</a>&nbsp; 
            <%} %> 
             
            <%for(int a=startpage ; a<=endpage ; a++){  
                    if(a==nowpage){ %>  
                       <a href="./NoticeList2.bo?page=<%=a %>&board_subject=<%= board_subject%>"><strong><%=a%></strong></a>&nbsp;  
               <%}else{ %> 
                <a href="./NoticeList2.bo?page=<%=a %>&board_subject=<%= board_subject%>"><strong><%=a%></strong></a>&nbsp; 
                <%} %> 
             <%} %>  
              
             <%if(nowpage>=maxpage){ %> <a href ="./NoticeList2.bo?page=<%=nowpage+1%>&board_subject=<%= board_subject%>" class="direction"><span>Next</span>
             </a>
            <%}else{ %> 
                <a href ="./NoticeList2.bo?page=<%=nowpage+1%>&board_subject=<%= board_subject%>">Next</a> 
            <%} %>
   ...
   <tr align="right"> 
        <td colspan="5"> 
			<% String boardsubject = request.getParameter("board_subject1"); %>    
        		<form action="./NoticeList2.bo?page=<%=nowpage%> method="POST">
